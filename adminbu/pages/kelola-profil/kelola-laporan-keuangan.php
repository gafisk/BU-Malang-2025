...
if ($id_lapkeu > 0) {
  $stmt = $conn->prepare("SELECT nama_laporan, isi_berita, judul_link, link FROM lap_keuangan WHERE id_lap_keuangan = ?");
  $stmt->bind_param("i", $id_lapkeu);
  $stmt->execute();
  $result = $stmt->get_result();

  if ($row = $result->fetch_assoc()) {
    $nama_lapooran = $row['nama_laporan'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $nama_lapooran = trim($_POST['nama_laporan']);
  $isi_berita = trim($_POST['isi_berita']);
  $judul_link = trim($_POST['judul_link']);
  $link = trim($_POST['link']);

  if (!empty($nama_lapooran) && !empty($isi_berita) && !empty($judul_link) && !empty($link)) {
    if ($id_lapkeu > 0) {
      // Mode Edit
      $stmt = $conn->prepare("UPDATE lap_keuangan SET nama_laporan = ?, isi_berita = ?, judul_link = ?, link = ?, waktu = NOW() WHERE id_lap_keuangan = ?");
      $stmt->bind_param("ssssi", $nama_lapooran, $isi_berita, $judul_link, $link, $id_lapkeu);
      if ($stmt->execute()) {
      ...
      // Mode Tambah Baru
      $stmt = $conn->prepare("INSERT INTO lap_keuangan (waktu, nama_laporan, isi_berita, judul_link, link) VALUES (NOW(), ?, ?, ?, ?)");
      $stmt->bind_param("ssss", $nama_lapooran, $isi_berita, $judul_link, $link);
              ...
              <form method="POST" action="">
                <div class="card-body">
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="nama_laporan" id="input_namalaporan" value="<?= htmlspecialchars($nama_lapooran); ?>" placeholder="Laporan Keuangan ....." required />
                  </div>
                  <div class="mb-3">
