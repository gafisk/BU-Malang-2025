...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['hapus']) && isset($_POST['id_pel_awardee'])) {
  $id_pel_awardee = intval($_POST['id_pel_awardee']);

  // Hapus data dari database
  $stmt = $conn->prepare("DELETE FROM pel_awardee WHERE id_pel_awardee = ?");
    ...
    <main class="app-main">
      ...
      <div class="app-content-header">
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            <div class="col-sm-6">
              <h3 class="mb-0">Daftar Pengumuman Pelaporan Awardee</h3>
            </div>
            <div class="col-sm-6">
              ...
              <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                  <tr>
                    ...
                    <td><?= date('d-m-Y H:i A', strtotime($row['batas_pelaporan'])); ?></td>
                    <td><?= htmlspecialchars($row['link']); ?></td>
                    <td><?= htmlspecialchars($row['narahubung']); ?></td>
                    <td><?= date('d-m-Y', strtotime($row['waktu'])); ?></td>
                    <td>
                      <a href="/BU-Malang-2025/adminbu/pages/kelola-pelaporan/kelola-pelaporan-awardee.php?id=<?= $row['id_pel_awardee']; ?>" class="btn btn-warning btn-sm">
                        <i class="bi bi-pencil"></i> </a>
                      <form method="POST" style="display:inline;" onsubmit="return confirmDelete();">
                        <input type="hidden" name="id_pel_awardee" value="<?= $row['id_pel_awardee']; ?>">
